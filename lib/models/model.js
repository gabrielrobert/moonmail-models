  ...
  _createClass(Model, null, [{
        ...
        (function () {
          ...
          Object.keys(options.filters).forEach(function (key) {
            ...
            Object.keys(conditions).forEach(function (operand) {
              ...
              filterExpressions.push(_this2._buildFilter(attrName, attrValue, _this2._filterOperandsMapping[operand]));
            });
          });
    ...
    value: function _refineItems(items, options) {
      ...
      if (String(options.include_fields) === 'false' && options.fields) {
        return items.map(function (item) {
          return _this3._refineItem(item, options);
        });
      } else {
        return items;
      }
    ...
    value: function update(params, hash, range) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var dbParams = {
          ...
          AttributeUpdates: _this4._buildAttributeUpdates(params),
          ReturnValues: 'ALL_NEW'
        };
        _this4._client('update', dbParams).then(function (result) {
    ...
    value: function _delete(hash, range) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this5._client('delete', params).then(function () {
          return resolve(true);
        }).catch(function (err) {
          return reject(err);
        });
    ...
    value: function allBy(key, value) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var dbOptions = _this6._buildOptions(options, params);
        (0, _deepAssign2.default)(params, dbOptions);
        _this6._client('query', params).then(function (result) {
          var response = _this6._buildResponse(result, params, options);
          resolve(response);
        }).catch(function (err) {
          return reject(err);
        });
    ...
    value: function _client(method, params) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this8._db()[method](params, function (err, data) {
          if (err) {
            (0, _logger.debug)('= Model._client', method, 'Error', err);
            reject(err);
          } else {
            (0, _logger.debug)('= Model._client', method, 'Success');
            if (data.UnprocessedItems && Object.keys(data.UnprocessedItems).length > 0 && retries < _this8.maxRetries) {
              (function () {
                ...
                var delay = _this8.retryDelay * Math.pow(2, retries);
                setTimeout(function () {
                  resolve(_this8._client(method, retryParams, retries + 1));
                }, delay);
              })();
            } else {
