...
var Model = function () {
  ...
  _createClass(Model, null, [{
    ...
    value: function _buildOptions(options) {
      ...
      var limitOptions = this._buildLimitOptions(options);
      var pageOptions = this._buildPageOptions(options);
      (0, _deepAssign2.default)(fieldsOptions, limitOptions, pageOptions);
    ...
    value: function _buildPaginationKey(result, params) {
      ...
      if (items && items.length > 0) {
        var _pageItem = items[0];
        var _tempKey = {};
        _tempKey[this.hashKey] = _pageItem[this.hashKey];
        if (this.rangeKey) {
          _tempKey[this.rangeKey] = _pageItem[this.rangeKey];
        }
        paginationKey.prevPage = this.prevPage(_tempKey);
      }
    ...
    value: function allBy(key, value) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this5._client('query', params).then(function (result) {
          var response = _this5._buildResponse(result, params, options);
          resolve(response);
        }).catch(function (err) {
          return reject(err);
        });
