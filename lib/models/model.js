...
var Model = function () {
  ...
  _createClass(Model, null, [{
    ...
    key: 'filterBy',
    value: function filterBy(key, value, opts, filters) {
      var _this7 = this;

      (0, _logger.debug)('= Model.filterBy', key, value, opts, filters);
      var options = (0, _omitEmpty2.default)(Object.assign({}, opts, { filters: filters }));
      return this._getAllBy(key, value, options).then(function (resultSet) {
        return _this7._recursiveFilterBy(key, value, options, 40, resultSet);
      }).then(function (resultSet) {
        var size = Math.min(options.limit, resultSet.Items.length);
        var toReturn = Object.assign({}, resultSet, { Items: resultSet.Items.slice(0, size) });
        // Build response
        var params = _this7._buildDynamodDBParams(key, value, options);
        var response = _this7._buildResponse(toReturn, params, options);
    ...
    key: '_recursiveFilterBy',
    value: function _recursiveFilterBy(key, value, options, iterations, results) {
      var _this8 = this;

      (0, _logger.debug)('= Model._recursiveFilterBy', key, value, options, iterations, results);
      ...
      return this._getAllBy(key, value, Object.assign({}, options, { page: this.nextPage(results.LastEvaluatedKey) })).then(function (resultSet) {
        ...
        return _this8._recursiveFilterBy(key, value, options, iterations - 1, aggregatedResults);
      });
    }
  }, {
    key: '_getAllBy',
    value: function _getAllBy(key, value) {
      ...
      return new Promise(function (resolve) {
        ...
        if (key !== _this9.hashKey) {
          options.gsiHashKey = key;
        }
        var params = _this9._buildDynamodDBParams(key, value, options);
        (0, _logger.debug)('= Model._getAllBy', key, value, options);
        resolve(_this9._client('query', params));
      });
    }
  }, {
    key: '_buildDynamodDBParams',
    value: function _buildDynamodDBParams(key, value, options) {
      var params = {
        TableName: this.tableName,
        ScanIndexForward: this.scanForward
      };
    ...
    value: function allBy(key, value) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var params = _this10._buildDynamodDBParams(key, value, options);
        _this10._getAllBy(key, value, options).then(function (result) {
          var response = _this10._buildResponse(result, params, options);
          resolve(response);
        }).catch(function (err) {
          return reject(err);
        });
